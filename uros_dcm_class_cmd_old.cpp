...
const float position_dead_zone = 0.0f; 

// Unos 0.86 grados de tolerancia (un poco más para el motor 2 si quieres)
const float position_dead_zone_2 = 0.0f; 
...
//     const float dt = 0.1f;  // 100 ms control period
   
//     // ========== MOTOR 1: CASCADA SUAVE (Kp=0.1) ==========
...
//     // 4. SUAVE correction (Kp=0.1, not 2.0)
//     // float vel_correction_1 = 7.0f * pos_error_1;
    
//     // Limit correction to ±0.2 rad/s to not perturb trajectory
//     // if (vel_correction_1 > 0.2f) vel_correction_1 = 0.2f;
//     // if (vel_correction_1 < -0.2f) vel_correction_1 = -0.2f;
   
//     // 5. Combine feedforward with soft correction
//     float vel_setpoint_1 = desired_vel_1 + vel_correction_1;
   
//     // 6. Limit total velocity to avoid aggressive moves
...
//     // float vel_correction_2 = 0.1f * pos_error_2;
//     // if (vel_correction_2 > 0.2f) vel_correction_2 = 0.2f;
//     // if (vel_correction_2 < -0.2f) vel_correction_2 = -0.2f;
...
//     // servo1.set_position(desired_servo);
//     float angle_for_servo = servo1.map_linear_to_angle(desired_servo);
//     //si no se mueve el servo
//     //float angle_for_servo2 = fabs(angle_for_servo)
...
//     debug_msg.linear.z = desired_vel_1;          // Desired velocity motor 1 (feedforward)
//     debug_msg.angular.x = pos_actual_2;          // Current position motor 2
//     debug_msg.angular.y = filtered_vel_1;        // Measured velocity motor 1
//     debug_msg.angular.z = desired_vel_2;         // Desired velocity motor 2 (feedforward)
...
void control_timer_callback(rcl_timer_t *timer, int64_t last_call_time) {
    ...
    // ========================================
    const float ERROR_THRESHOLD_PID = 0.5f;      // 0.5 rad (~28.6°) - Umbral para PID completo
    const float ERROR_THRESHOLD_STOP = 0.03f;   // 0.015 rad (~0.86°) - Zona muerta final
    const float MIN_PWM_APPROACH = 15.0f;         // PWM mínimo para acercamiento (ajustar según tu motor)
